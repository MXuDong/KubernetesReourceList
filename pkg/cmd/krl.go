...
func (o *KRLOptions) Run() error {
	...
	t := Table{}
	t.InitTitle([]string{"name", "namespace", "api-group", "api-kind"})
	t.SetIndexAlignStyle(0, LeftAlignType)
	err := result.Visit(func(info *resource.Info, err error) error {
		name := info.Name
		namespace := info.Namespace
		apiGroup := info.Mapping.GroupVersionKind.Group
		apiKind := info.Mapping.GroupVersionKind.Kind
		t.addRows([]string{name, namespace, apiGroup, apiKind})
		return nil
	})
	fmt.Println(t.DoPrint())
